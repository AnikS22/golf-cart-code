...
│    - receives 0x130 BRAKE_CMD → J1939 PGN 65280 on CAN3 → Kartech actuator│
...
3. Pedals Teensy receives, calls kartech::send_brake_permil() at 50 Hz on CAN3.
4. Driver maps permil → 14-bit Kartech position in the useful band
     [POS_RELEASE_STOCK=3520 .. POS_FULL_BRAKE=3009]   # ~510 units of travel
   and TXs J1939 frame {0x0F, 0x4A, LSB, MSB|0xC0, 0,0,0,0} on PGN 65280.
5. Kartech 1A001HAJ runs its own closed-loop servo to the target position.
6. Cable pulls brake pedal lever; pedal pivots; master cylinder applies hydraulic pressure.
7. Driver can always press deeper (mechanical override) — actuator is just a parallel cable pull.
```
