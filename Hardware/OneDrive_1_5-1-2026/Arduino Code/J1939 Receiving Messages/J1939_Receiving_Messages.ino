...
void loop() 
{
  ...
  {
        // sprintf(sString, "PGN: %ld Src: 0x%X Dest: 0x%X ", (int)lPGN, nSrcAddr, nDestAddr);
        ...
        {if(PGN == (int)lPGN )
        {
          sprintf(sString, "PGN: %li, Dest: 0x%X", (long)lPGN,nDestAddr );
          Serial.print(sString);
          Serial.print("Data: ");
          for(int nIndex = 0; nIndex < nDataLen; nIndex++)
          {          
            sprintf(sString, "%d ", nData[nIndex]);
            Serial.print(sString);
